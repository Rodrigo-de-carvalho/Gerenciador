...
function parseAmount(str) {
  ...
  // Normalize various dash/minus characters (em-dash U+2212, en-dash U+2013, etc.)
  // Nubank sometimes exports with the mathematical minus sign instead of hyphen
  s = s.replace(/[−–—―−]/g, '-');
  s = s.replace(/[R$\s]/g, '');
...
export function parseRows(bank, data) {
  switch (bank) {
    ...
    case 'nubank_conta': {
      // Nubank account CSV sign convention:
      //   positive amount → income (credit/pix received)
      //   negative amount → expense (debit/payment)
      return data.map(r => {
        const dateKey   = findKey(r, 'data', 'date');
        const valKey    = findKey(r, 'valor', 'value', 'amount');
        const descKey   = findKey(r, 'descri', 'titulo', 'hist', 'title');
        const amount    = parseAmount(valKey ? r[valKey] : null);
        if (amount === null) return null;
        return {
          date:         parseDateBR(dateKey ? r[dateKey] : null),
