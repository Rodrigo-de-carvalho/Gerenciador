...
import {
  ...
  ChevronLeft, ChevronRight, Share2, ExternalLink, Phone
} from 'lucide-react';
import {
  ...
  const handleWhatsAppAPI = async () => {
    ...
    try {
      const res = await fetch('https://api.z-api.io/instances/YOUR_INSTANCE/token/YOUR_TOKEN/send-text', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Client-Token': whatsappApiKeyInput },
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button className="btn-success text-sm py-2 px-3" onClick={() => exportToExcel(monthTxs, categories, month, year)} disabled={monthTxs.length === 0}>
            <FileSpreadsheet className="w-4 h-4" />
            <span className="hidden sm:inline">Excel</span>
          </button>
          <button className="btn-danger text-sm py-2 px-3" onClick={() => downloadPDF(monthTxs, categories, month, year)} disabled={monthTxs.length === 0}>
            <FileText className="w-4 h-4" />
            <span className="hidden sm:inline">PDF</span>
          </button>
          <button className="btn-primary text-sm py-2 px-3" onClick={() => setShowWhatsappPanel(p => !p)}>
      ...
      {/* WhatsApp panel */}
      {showWhatsappPanel && (
        <div className="card border-2 border-green-200 dark:border-green-800 bg-green-50/50 dark:bg-green-900/10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="space-y-4">
              <div className="bg-white dark:bg-slate-700 rounded-xl p-4 border border-slate-200 dark:border-slate-600">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">Abre o WhatsApp com a mensagem pronta.</p>
              ...
              <div className="bg-white dark:bg-slate-700 rounded-xl p-4 border border-slate-200 dark:border-slate-600">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">
                  Configure o endpoint em <code className="bg-slate-100 dark:bg-slate-600 px-1 rounded">exportUtils.js</code>.
                </p>
                <input type="tel" className="input-field mb-2" placeholder="Número destino" value={whatsappPhone} onChange={e => setWhatsappPhone(e.target.value)} />
                <input type="password" className="input-field mb-2" placeholder="Client-Token da API" value={whatsappApiKeyInput} onChange={e => setWhatsappApiKeyInput(e.target.value)} />
                <button className="btn-primary w-full justify-center text-sm" onClick={handleWhatsAppAPI}>
                  <MessageSquare className="w-4 h-4" /> Enviar via API
                </button>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { label: 'Total Entradas', value: income, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-900/20', border: 'border-emerald-200 dark:border-emerald-800' },
              { label: 'Total Saídas', value: expense, color: 'text-red-600', bg: 'bg-red-50 dark:bg-red-900/20', border: 'border-red-200 dark:border-red-800' },
              { label: 'Saldo Líquido', value: balance, color: balance >= 0 ? 'text-blue-600' : 'text-orange-600', bg: 'bg-blue-50 dark:bg-blue-900/20', border: 'border-blue-200 dark:border-blue-800' },
              { label: 'Taxa de Poupança', value: null, color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-900/20', border: 'border-purple-200 dark:border-purple-800', raw: `${savingsRate}%` },
            ].map((kpi, i) => (
              <div key={i} className={`${kpi.bg} border ${kpi.border} rounded-2xl p-4`}>
                ...
                <p className={`text-base sm:text-lg font-bold ${kpi.color}`}>
          ...
          <div className="card">
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm mb-4">Entradas vs Saídas por Mês</h3>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={monthlyData.slice(-6)} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis tick={{ fontSize: 10, fill: tickColor }} axisLine={false} tickLine={false} tickFormatter={v => `R$${v >= 1000 ? (v/1000).toFixed(0)+'k' : v}`} width={55} />
          ...
          <div className="card">
            ...
            {monthTxs.length === 0 ? (
              <p className="text-center text-slate-400 dark:text-slate-500 py-8">Nenhum lançamento neste período.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm min-w-[400px]">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-700">
                      <th className="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 pb-2 pr-4">Data</th>
                      <th className="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 pb-2 pr-4">Descrição</th>
                      <th className="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 pb-2 pr-4 hidden sm:table-cell">Categoria</th>
                      <th className="text-right text-xs font-semibold text-slate-500 dark:text-slate-400 pb-2">Valor</th>
                    </tr>
                  </thead>
                    ...
                    {[...monthTxs].sort((a, b) => new Date(b.date) - new Date(a.date)).map(t => {
                      ...
                      return (
                        <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40">
                          <td className="py-2 pr-4 text-slate-500 dark:text-slate-400 whitespace-nowrap text-xs">{formatDate(t.date)}</td>
                          <td className="py-2 pr-4 text-slate-700 dark:text-slate-200 max-w-[150px] truncate">{t.description}</td>
                          ...
                          <td className={`py-2 text-right font-semibold text-xs sm:text-sm ${t.type === 'income' ? 'text-emerald-600' : 'text-red-600'}`}>
      ...
      {activeTab === 'categories' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="card">
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm mb-4">Distribuição das Saídas</h3>
            {catBreakdown.length > 0 ? (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={catBreakdown} cx="50%" cy="45%" outerRadius={85} innerRadius={48} dataKey="value" paddingAngle={3}>
              ...
              <div className="h-[260px] flex items-center justify-center text-slate-400 dark:text-slate-500 text-sm">Sem saídas este mês</div>
            )}
          </div>

          <div className="card">
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm mb-4">Ranking de Gastos</h3>
            <div className="space-y-3">
              {catBreakdown.length === 0 ? (
                <p className="text-slate-400 dark:text-slate-500 text-sm text-center py-8">Sem dados</p>
              ) : (
                    ...
                    <div key={cat.name}>
                      <div className="flex items-center justify-between text-sm mb-1">
                        ...
                        <div className="text-right">
                          <span className="font-bold text-slate-700 dark:text-slate-200 text-xs sm:text-sm">{formatCurrency(cat.value)}</span>
      ...
      {activeTab === 'evolution' && (
        <div className="space-y-5">
          <div className="card">
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm mb-4">Evolução do Saldo (12 meses)</h3>
            <ResponsiveContainer width="100%" height={230}>
              <LineChart data={monthlyData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis tick={{ fontSize: 9, fill: tickColor }} axisLine={false} tickLine={false} tickFormatter={v => `R$${v >= 1000 ? (v/1000).toFixed(0)+'k' : v}`} width={52} />
                <Tooltip content={<CustomTooltip />} />
          ...
          <div className="card overflow-x-auto">
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 text-sm mb-4">Tabela Anual</h3>
            <table className="w-full text-sm min-w-[380px]">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-700">
                  <th className="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 pb-2 pr-4">Mês</th>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                {monthlyData.map((row, i) => (
                  <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/40">
                    <td className="py-2 pr-4 font-medium text-slate-700 dark:text-slate-200 text-xs sm:text-sm">{row.name}</td>
                    <td className="py-2 pr-4 text-right text-emerald-600 font-medium text-xs sm:text-sm">{formatCurrency(row.Entradas)}</td>
                    <td className="py-2 pr-4 text-right text-red-600 font-medium text-xs sm:text-sm">{formatCurrency(row.Saídas)}</td>
                    <td className={`py-2 text-right font-bold text-xs sm:text-sm ${row.Saldo >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
              ...
              <tfoot>
                <tr className="border-t-2 border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700/50">
                  <td className="py-2 pr-4 font-bold text-slate-700 dark:text-slate-200 text-xs sm:text-sm">TOTAL</td>
                  <td className="py-2 pr-4 text-right font-bold text-emerald-600 text-xs sm:text-sm">{formatCurrency(monthlyData.reduce((s, r) => s + r.Entradas, 0))}</td>
                  <td className="py-2 pr-4 text-right font-bold text-red-600 text-xs sm:text-sm">{formatCurrency(monthlyData.reduce((s, r) => s + r.Saídas, 0))}</td>
                  <td className={`py-2 text-right font-bold text-xs sm:text-sm ${monthlyData.reduce((s, r) => s + r.Saldo, 0) >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
